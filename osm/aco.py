...
class TraciClient():
    def __init__(self, traciLabel, sumoBinary,
    ...
    def random_routing_from_edge_neighbors(self, vehID):
        ...
        if currentIndexOnRoute == -1 or currentIndexOnRoute == len(vehRoute) - 1:
            ...
            if currentEdgeID in self.G and self.G.nodes[currentEdgeID]['is_internal'] is False:
                nextEdgeID = random.choice(list(self.G[currentEdgeID].keys()))
                try:
    ...
    def wait_for_vehicles_to_load(self, stopAfterLoading=True):
        loadedVehicles = 0
        while True:
            # while self.traciConnection.simulation.getMinExpectedNumber() > 0 and (stopAfterLoading and loadedVehicles < self.totalVehiclesToLoad):
            self.traciConnection.simulationStep()
    ...
    def initialize_global_variables(self):
        ...
        # self.edges = list(self.G.nodes.keys())
    ...
    def run(self):
        ...
        self.timeStep = 1
        self.wait_for_vehicles_to_load(stopAfterLoading=False)
        # self.initTravelInfo()
...
if __name__ == "__main__":
    ...
    client1 = TraciClient(str(1), sumoBinary,
